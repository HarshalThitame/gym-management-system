...
export default function SmsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error("SMS page error:", error); }, [error]);
  ...
  return (
    <div className="flex items-center justify-center py-24">
      <div className="max-w-md text-center">
        ...
        <h2 className="mt-5 text-xl font-black">Failed to load SMS data</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          {error.message || "An unexpected error occurred while loading SMS information."}
        </p>
