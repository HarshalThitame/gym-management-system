...
export default async function MemberAiCoachPage() {
  const context = await requireMemberPortalAccess("/member/ai-coach");
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="AI Fitness Intelligence" title="Personal coaching with human supervision" description="AI analyzes your goals, attendance, workouts, measurements, nutrition logs, and class participation to suggest next actions. Staff review is required for plan changes." />
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
        <AnimatedCardSection delay={0.1}>
          <Card variant="glass">
            ...
            <CardContent><AiCoachChatPanel /></CardContent>
          </Card>
        </AnimatedCardSection>
        ...
        <Card variant="glass">
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {(dashboard?.recommendations ?? []).map((recommendation) => (
              <div className="rounded-xl border border-border bg-surface-muted p-4 card-hover" key={recommendation.title}>
            ...
            {(dashboard?.storedRecommendations ?? []).slice(0, 3).map((recommendation) => (
              <div className="rounded-xl border border-border bg-surface p-4 card-hover" key={recommendation.id}>
