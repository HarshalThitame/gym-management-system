...
export default async function MemberFitnessPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-xs font-black uppercase tracking-[0.14em] text-muted-foreground">Fitness Progress</p>
        <h2 className="mt-2 text-3xl font-black">Training, nutrition, and body progress</h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Log workouts, track body measurements, compare progress photos, follow nutrition plans, and keep goals visible.</p>
      </div>
            ...
            {portal.goals.map((goal) => (
              <div className="rounded-lg border border-border bg-surface-muted p-4" key={goal.id}>
                ...
                <div className="mt-4 h-2 overflow-hidden rounded-full bg-surface">
                  <div className="h-full rounded-full bg-primary" style={{ width: `${calculateGoalProgress(goal)}%` }} />
                </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[0.9fr_1fr]">
        ...
        <Card>
          <CardHeader>
            <h3 className="text-2xl font-black">Workout History</h3>
          </CardHeader>
          <CardContent className="space-y-4">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2"><Apple className="size-5" /><h3 className="text-2xl font-black">Nutrition</h3></div>
          </CardHeader>
          <CardContent className="space-y-4">
            <MealEntryForm memberId={portal.member.id} nutritionPlans={portal.nutritionPlans} />
            <div className="space-y-3">
              {portal.mealEntries.slice(0, 8).map((entry) => (
                <div className="rounded-md border border-border bg-surface-muted p-3" key={entry.id}>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2"><Camera className="size-5" /><h3 className="text-2xl font-black">Progress Photos</h3></div>
          </CardHeader>
          <CardContent className="space-y-4">
            <ProgressPhotoForm memberId={portal.member.id} />
            <div className="grid gap-3 md:grid-cols-3">
              {portal.progressPhotos.slice(0, 6).map((photo) => (
                <div className="rounded-md border border-border bg-surface-muted p-3" key={photo.id}>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {portal.milestones.map((milestone) => (
              <div className="rounded-md border border-border bg-surface-muted p-3" key={milestone.id}>
