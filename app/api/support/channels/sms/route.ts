...
export async function POST(request: Request) {
  try {
    ...
    if (!apiKey || apiKey !== process.env.SUPPORT_SMS_WEBHOOK_KEY) {
    ...
    if (!ticket) {
      return NextResponse.json({ ok: false, error: { code: "NO_OPEN_TICKET", message: "No open ticket found." } }, { status: 404 });
    }

    const msgPayload: Record<string, unknown> = { channel: "sms", direction: "inbound", senderName: fromNumber, body: text };
    if (body.messageId) msgPayload.externalId = body.messageId;
    await addMessage(ticket.id, msgPayload as never);

    return NextResponse.json({ ok: true, message: "SMS processed." });
  } catch (e) {
    return NextResponse.json({ ok: false, error: { code: "WEBHOOK_ERROR", message: e instanceof Error ? e.message : "Failed to process SMS." } }, { status: 500 });
  }
}
