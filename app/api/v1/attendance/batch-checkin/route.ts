...
import { batchCheckInMembersV1, resolveGymScopeIds } from "@/features/attendance/lib/phase1-api";
...
export const POST = withApiAuth(async (request: NextRequest, context) => {
  ...
  const source = typeof body.source === "string" ? body.source : typeof body.checkInSource === "string" ? body.checkInSource : null;
  ...
  if (gymIds.length !== 1) {
    return NextResponse.json({ ok: false, error: { code: "GYM_SCOPE_REQUIRED", message: "Provide a single gymId for batch check-in." } }, { status: 400 });
  }

  const result = await batchCheckInMembersV1({
    actor: {
