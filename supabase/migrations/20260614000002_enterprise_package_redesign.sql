...
select
  ...
  coalesce((select pf.value #>> '{}' from public.package_features pf where pf.package_id = p.id and pf.feature_code = 'face_recognition_attendance')::boolean, false) as face_recognition_attendance_enabled,
  ...
  coalesce((select pf.value #>> '{}' from public.package_features pf where pf.package_id = p.id and pf.feature_code = 'franchise_management')::boolean, false) as franchise_management_enabled,
...
insert into public.feature_catalog (code, name, description, category_id, feature_type, default_value, sort_order) values
  ...
  ('face_recognition_attendance', 'Face Recognition', 'AI-powered face recognition check-in', (select id from cats where code = 'attendance'), 'boolean', 'false', 9),
  ...
  -- Enterprise features
  ('franchise_management', 'Franchise Management', 'Multi-location franchise management', (select id from cats where code = 'enterprise'), 'boolean', 'false', 1),
...
begin
  for p in select * from public.packages loop
    ...
    insert into public.package_features (package_id, feature_code, value) values
      ...
      (p.id, 'franchise_management', to_jsonb(false)),
      ...
      (p.id, 'face_recognition_attendance', to_jsonb(false)),
...
begin
  ...
  insert into public.package_features (package_id, feature_code, value) values
    ...
    (v_pkg_id, 'face_recognition_attendance', 'true'),
    (v_pkg_id, 'rfid_attendance', 'true'),
    (v_pkg_id, 'nfc_attendance', 'true'),
    ...
    (v_pkg_id, 'multi_branch_management', 'true'),
    (v_pkg_id, 'franchise_management', 'true'),
