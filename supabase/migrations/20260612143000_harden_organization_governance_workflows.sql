-- Harden Super Admin organization governance workflows.
-- This migration makes approval application atomic, adds automatic expiry,
-- and supports legal-hold-aware permanent purge requests while preserving
-- an auditable tenant tombstone instead of deleting governance evidence.

alter table public.organization_approval_requests
drop constraint if exists organization_approval_requests_action_check;

alter table public.organization_approval_requests
add constraint organization_approval_requests_action_check
check (
  action in (
    'transfer_owner',
    'suspend',
...
create index if not exists organization_approval_requests_status_expires_idx
on public.organization_approval_requests (status, expires_at);

create or replace function public.expire_organization_approval_requests(p_actor_id uuid default null)
returns integer
language plpgsql
security definer
...
begin
  if auth.role() <> 'service_role' and not public.is_super_admin() then
    raise exception 'Only Super Admins can expire organization approvals.'
      using errcode = '42501';
  end if;
...
begin
  ...
  if v_approval.requested_by is not null and v_approval.requested_by = p_reviewer_id then
    raise exception 'Maker-checker control blocked this approval. A different Super Admin must approve the request.'
      using errcode = '42501';
  end if;
...
grant execute on function public.expire_organization_approval_requests(uuid) to authenticated, service_role;
grant execute on function public.apply_organization_approval_request(uuid, uuid, text) to authenticated, service_role;

comment on function public.apply_organization_approval_request(uuid, uuid, text) is
  'Atomically applies a pending Super Admin organization approval and marks it approved under row locks.';
comment on function public.expire_organization_approval_requests(uuid) is
  'Expires pending organization approval requests whose review window has elapsed.';
