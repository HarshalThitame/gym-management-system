...
export function renderBrandedEmail({ title, preview, body, ctaLabel, ctaUrl }: BrandedEmailInput) {
  const cta = ctaLabel && ctaUrl
    ? `<p style="margin:28px 0 0"><a href="${ctaUrl}" style="display:inline-block;background:#111214;color:#ffffff;text-decoration:none;padding:13px 18px;border-radius:6px;font-weight:700">${ctaLabel}</a></p>`
    : "";
...
export function welcomeEmail(fullName: string) {
  return renderBrandedEmail({
    ...
    body: `<p>Hi ${fullName || "there"}, your member portal account is ready. Verify your email from the Supabase confirmation message, then sign in to manage your profile, membership, attendance, and class bookings.</p>`,
...
export function passwordChangedEmail(fullName: string) {
  return renderBrandedEmail({
    ...
    body: `<p>Hi ${fullName || "there"}, the password for your Apex account was changed. If you did not make this change, contact the gym team immediately.</p>`
  });
}
...
export function accountCreatedEmail(fullName: string, loginUrl: string) {
  return renderBrandedEmail({
    ...
    body: `<p>Hi ${fullName || "there"}, an Apex account has been created for you. Use the secure login page to complete access setup and verify your email before using staff tools.</p>`,
