...
async function buildExecutiveAnalyticsDashboard(supabase: SupabaseClient<Database>, gymId: string | null): Promise<ExecutiveAnalyticsDashboard> {
  ...
  return {
    ...
    membershipAnalytics: {
      ...
      newMemberships: newMembersThisMonth,
      renewals: renewalsThisMonth,
      freezes: 0,
      expirations: expiredMemberships.length,
      cancellations: cancelledMemberships.length,
      cohorts: [],
      planPopularity
    },
    attendanceTrend,
    ...
    ltvAnalytics: {
      currentLtv: estimatedLifetimeValue,
      predictedLtv: estimatedLifetimeValue,
      averageLtv: estimatedLifetimeValue,
      segments: [],
      segmentRevenue: []
    },
    executiveSummary: {
      ...
      churnRate,
      retentionRate,
      lifetimeValue: estimatedLifetimeValue,
      arpm: activeMemberships.length > 0 ? monthRevenue / activeMemberships.length : 0,
      cac: 0,
      nrr: 100,
      expansionRevenue: 0,
      refundRate: 0,
      occupancyUtilization: averageClassUtilization,
      branchPerformanceIndex: 0
    },
    revenueIntelligence: {
      mrr: monthRevenue,
      arr: monthRevenue * 12,
      deferredRevenue: 0,
      collectedRevenue: monthRevenue,
      ...
      revenueByPlan: [],
      revenueByService: [],
      revenueBySource: revenueSources.map((s) => ({ name: s.source, amount: s.amount }))
    },
    branchScorecards: [],
    branchRanking: { topPerformers: [], underperformers: [], growthLeaders: [] },
    marketingAnalytics: {
      campaigns: [], channelAnalysis: [], attribution: [],
      totalLeads: leads.length, totalConversions: leads.filter((l) => l.status === "converted").length,
      totalRevenue: monthRevenue, totalCac: 0, totalRoi: 0
    },
    capacityAnalytics: { occupancyRates: [], peakHours: [], overcrowdingRisk: [] },
    behaviorAnalytics: { segments: [], journey: [], engagementTrend: [] },
    forecastScenarios: [],
    alerts: [],
    alertHistory: [],
    forecasts,
