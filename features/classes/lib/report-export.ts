...
import { getClassReportTable, type ClassReportPayload, type ClassReportTable } from "./csv";

export const classReportFormats = ["csv", "excel", "pdf"] as const;
export type ClassReportFormat = (typeof classReportFormats)[number];

export function classRowsToExcel(report: ClassReportPayload) {
...
export async function classRowsToPdf(report: ClassReportPayload) {
  const table = getClassReportTable(report);
...
function drawHeader(page: PDFPage, table: ClassReportTable, bold: PDFFont, regular: PDFFont, pageSize: [number, number], margin: number) {
  ...
  page.drawText("CLASS REPORT", { x: pageSize[0] - margin - 132, y, size: 14, font: bold, color: rgb(0.067, 0.071, 0.078) });
...
function drawTableHeader(page: PDFPage, table: ClassReportTable, bold: PDFFont, columnWidth: number, margin: number, y: number, height: number) {
