...
export async function sendCampaignEmail(params: {
  ...
  const r = await resendSendEmail({ to: params.to, subject: params.subject, html: params.body });
...
export async function sendViaChannel(channel: string, recipient: string, subject: string, body: string): Promise<SendResult> {
  switch (channel) {
    case "email":
      return sendCampaignEmail({ to: recipient, subject, body });
    case "sms":
