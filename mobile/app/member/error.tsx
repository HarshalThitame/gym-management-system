...
export default function MemberErrorBoundary({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.bg, alignItems: "center", justifyContent: "center", padding: theme.spacing["2xl"] }}>
      ...
      <Text variant="h3" center style={{ marginTop: theme.spacing.xl, color: theme.colors.danger }}>Something went wrong</Text>
      <Text variant="body" muted center style={{ marginTop: theme.spacing.sm, maxWidth: 280 }}>
        {error.message ?? "An unexpected error occurred in the member portal."}
      </Text>
      <View style={{ flexDirection: "row", gap: theme.spacing.md, marginTop: theme.spacing["2xl"] }}>
        ...
        <TouchableOpacity onPress={reset} style={{ padding: theme.spacing.md, backgroundColor: theme.colors.primary, borderRadius: theme.radii.md, flexDirection: "row", alignItems: "center", gap: theme.spacing.sm }}>
          <RefreshCw size={18} color={theme.colors.primaryFg} />
          <Text variant="body" color={theme.colors.primaryFg}>Retry</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
